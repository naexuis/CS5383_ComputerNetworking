...
import replyMessage_pb2 as rpb
import replyMessage_pb2_grpc as rpb_grpc

##################################
#  The Service implementation class
##################################
class ServiceHandler (opb_grpc.OrderServiceServicer):
  ...
  def method (self, request, context):
    ...
    try:
      ...
      print ("Received request - seq no: {}, timestamp: {}, tomato: {}".format (request.seq_no, request.ts, request.veggies.tomato))
      print ("Received request:\n", request)


      # Now send response
      resp = rpb.ReplyMessage ()  # allocate the response object. Note it is empty
      if request.seq_no>=0:
        resp.reply_request = 1  # reply request status GOOD
        resp.reply_status = 1 # reply status GOOD
      else:
        resp.reply_request = 2 # reply request status BAD
        resp.reply_status = 2 # reply status BAD
...
def driver (port):
  ...
  try:
    ...
    opb_grpc.add_OrderServiceServicer_to_server(handler, server)
